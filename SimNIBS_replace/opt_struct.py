'''
    ...
    Modified by Xu Zhang, 2022 (each change is indicated in the script)
...
class TMSoptimize():
    ...
    def __init__(self, matlab_struct=None):
        ...
        # Modified by Xu Zhang, 2022 (start)
        self.custom_target_coords_option = False
        self.custom_target_coords_file = []
        self.custom_coil_matrices_option = False
        self.custom_coil_matrices_file = []
        self.record_coil_matrices_option = False
        self.record_coil_matrices_file = []
        self.resampleflag = True
        self.N_adm = [17,17,2]
        self.Xr = []
        self.Yr = []
        self.Zr = []
        # Modified by Xu Zhang, 2022 (end)
    ...
    def _prepare(self):
        ...
        if self.target is None or len(self.target) != 3:
            # Modified by Xu Zhang, 2022 (start)
            if self.custom_target_coords_option:
                target_points = genfromtxt(self.custom_target_coords_file, delimiter=',')
                self.target = np.mean(target_points, axis=0)
            else:
    ...
    def sim_struct2mat(self):
        ...
        # Modified by Xu Zhang, 2022 (start)
        mat['custom_target_coords_option'] = remove_None(self.custom_target_coords_option)
        mat['custom_target_coords_file'] = remove_None(self.custom_target_coords_file)
        mat['custom_coil_matrices_option'] = remove_None(self.custom_coil_matrices_option)
        mat['custom_coil_matrices_file'] = remove_None(self.custom_coil_matrices_file)
        mat['record_coil_matrices_option'] = remove_None(self.record_coil_matrices_option)
        mat['record_coil_matrices_file'] = remove_None(self.record_coil_matrices_file)
        mat['resampleflag'] = remove_None(self.resampleflag)
        mat['N_adm'] = remove_None(self.N_adm)
        mat['Xr'] = remove_None(self.Xr)
        mat['Yr'] = remove_None(self.Yr)
        mat['Zr'] = remove_None(self.Zr)
        # Modified by Xu Zhang, 2022 (end)
    ...
    def read_mat_struct(self, mat):
        ...
        # Modified by Xu Zhang, 2022 (start)
        self.custom_target_coords_option = try_to_read_matlab_field(
            mat, 'custom_target_coords_option', bool, self.custom_target_coords_option
        )
        self.custom_target_coords_file = try_to_read_matlab_field(
            mat, 'custom_target_coords_file', str, self.custom_target_coords_file
        )
        self.custom_coil_matrices_option = try_to_read_matlab_field(
            mat, 'custom_coil_matrices_option', bool, self.custom_coil_matrices_option
        )
        self.custom_coil_matrices_file = try_to_read_matlab_field(
            mat, 'custom_coil_matrices_file', str, self.custom_coil_matrices_file
        )
        self.record_coil_matrices_option = try_to_read_matlab_field(
            mat, 'record_coil_matrices_option', bool, self.record_coil_matrices_option
        )
        self.record_coil_matrices_file = try_to_read_matlab_field(
            mat, 'record_coil_matrices_file', str, self.record_coil_matrices_file
        )
        self.resampleflag = try_to_read_matlab_field(
            mat, 'resampleflag', bool, self.resampleflag
        )
        self.N_adm = try_to_read_matlab_field(
            mat, 'N_adm', list, self.N_adm
        )
        self.Xr = try_to_read_matlab_field(
            mat, 'Xr', list, self.Xr
        )
        self.Yr = try_to_read_matlab_field(
            mat, 'Yr', list, self.Yr
        )
        self.Zr = try_to_read_matlab_field(
            mat, 'Zr', list, self.Zr
        )
        # Modified by Xu Zhang, 2022 (end)
    ...
    def run(self, cpus=1, allow_multiple_runs=False, save_mat=True, return_n_max=1):
        ...
        # Modified by Xu Zhang, 2022 (start)
        if self.custom_coil_matrices_option:
            coil_matrices = genfromtxt(self.custom_coil_matrices_file, delimiter=',')
            pos_matrices = []
            for coil_matrices_num in range(round(len(coil_matrices)/4)):
                pos_matrices.append(coil_matrices[coil_matrices_num*4:coil_matrices_num*4+4,:])
        else:
            pos_matrices = self._get_coil_positions()
            if self.record_coil_matrices_option:
                pos_matrices_2d = [item for sublist in pos_matrices for item in sublist]
                with open(self.record_coil_matrices_file, 'w') as f:
                    for row in pos_matrices_2d:
                        f.write(' '.join(map(str, row)) + '\n')
            
        if self.custom_target_coords_option:
            target_region = self._get_target_region_predefined()
        else:
    ...
    # Modified by Xu Zhang, 2022 (start)
    def _get_target_region_predefined(self):
        return optimize_tms.define_target_region_predefined(
            self.mesh,
            self.custom_target_coords_file,
            self.tissues
        )
    ...
    def _ADM_optimize(self, cond_field, target_region):
        # Modified by Xu Zhang, 2022 (start)
        if self.custom_coil_matrices_option:
            tmp_matrices = genfromtxt(self.custom_coil_matrices_file, delimiter=',')
            coil_matrices = []
            for coil_matrices_num in range(round(len(tmp_matrices)/4)):
                coil_matrices.append(tmp_matrices[coil_matrices_num*4:coil_matrices_num*4+4,:])
            coil_matrices = np.array(coil_matrices).transpose(1, 2, 0)
            rotations = np.array([[0],[1],[0]]) # At least 1 auxillary dipole is required
        else:
        ...
        if self.target_direction is None:
            ...
            E_roi = ADMlib.ADMmag(
                ...
                coil_matrices, rotations, self.resampleflag, self.N_adm, self.Xr, self.Yr, self.Zr # Modified by Xu Zhang, 2024
            ) * self.didt
        else:
            if len(self.target_direction) != 3:
                raise ValueError('target direction should have 3 elements!')
